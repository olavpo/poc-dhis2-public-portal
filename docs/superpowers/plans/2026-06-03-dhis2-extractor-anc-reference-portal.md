...
- **API shapes (verified):**
  ...
  - `GET /api/geoFeatures.json?ou=ou:LEVEL-n` → array of `{ id, na, le, pi, pn, pg, ty, co }`. `ty` 1=point/facility, 2=polygon/area. `co` is a **JSON string**: point `"[lng,lat]"`, polygon `"[[[lng,lat],...]]"`. **Used for geometry only** (see below).
  - **Org-unit hierarchy comes from `/api/organisationUnits`, NOT geoFeatures** (corrected during implementation — commit `c3fb719`). geoFeatures omits geometry-less units (the national root), which broke the root-OU selector. `ou.csv` = hierarchy from `GET /api/organisationUnits.json?fields=id,name,level,parent[id,name],path&filter=level:in:[…]&paging=false` left-joined to geometry (`ty`,`lng`,`lat`) from geoFeatures by id. `path` is the DHIS2 form (`/root/.../self` — leading slash, **includes self**); the descendant-or-self scope predicate `('/' || o.path || '/') like '%/' || root || '/%'` works with it. See the spec's "Known pitfalls".
